...
public class UnionPayStrategy implements IPayStrategy {
    ...
    public String polymerizationPay(PaymentChannelEntity pymentChannel, PayMentTransacDTO payMentTransacDTO) {
        ...
        Long txnAmt =  payMentTransacDTO.getPayAmount();
        // 注：支付编码不是订单编码
        String orderId = payMentTransacDTO.getPaymentId();
        Date data = payMentTransacDTO.getCreatedTime();
        ...
        requestData.put("txnTime", sdf.format(data));        //订单发送时间，取系统时间，格式为 yyyyMMddHHmmss，必须取当前时间，否则会报txnTime无效
        ...
        requestData.put("riskRateInfo", "{commodityname=Dell 燃灵7000 商品}");
        ...
        requestData.put("payTimeout", sdf.format(System.currentTimeMillis() + 15 * 60 * 1000));
